...
then
	echo ""
	echo "TCP Tweaker network settings have been added to the system!"
	echo ""
...
sysctl -p /etc/sysctl.conf > /dev/null
	...
	echo "This script modifies some network settings"
	echo "in order to reduce latency and improve speed."
	echo ""
	read -p "You want to continue with the installation? [s/n]: " -e -i n resposta
	if [[ "$resposta" = 's' ]]; then
	echo ""
	echo "Modifying the following configurations:"
...
sysctl -p /etc/sysctl.conf
		echo ""
		echo "Network settings with TCP Tweaker were added correctly."
		echo ""
	else
		echo ""
