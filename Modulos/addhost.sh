...
then
	...
	then
		...
		then
			...
			if [[ $host != \.* ]]
			then
				tput setaf 7 ; tput setab 4 ; tput bold ; echo "" ; echo "You must add a domain starting with a period!" ; echo "For example: .phreaker56.xyz" ; echo "There is no need to add subdomains for domains that are already in the archive" ; echo "That is, it is not necessary to add recargawap.claro.com.br" ; echo "if the .claro.com.br domain is already in the file." ; echo ""; tput sgr0
				exit 1
			else
